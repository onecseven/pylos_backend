...
const distribute_state = (state, roomId) => send_to_room(messages.GAME_STATE(state), roomId)
...
const start_pylos = (user, data) => {
  ...
  if (room.isHost(user.id)) {
    ...
    room.users.forEach((user, i) => send_turn(user.id, i))
    room.game = create_pylos_game()
    distribute_state(room.game.serial_state, room.id)
  }
}
